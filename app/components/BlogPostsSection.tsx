...
const BlogPostsSection: React.FC<BlogPostsSectionProps> = ({ initialPosts }) => {
  const [posts] = useState<Post[]>(initialPosts);
  const [activeIndex, setActiveIndex] = useState(0);
  const [isAnimating, setIsAnimating] = useState(false);
  const carouselRef = useRef<HTMLDivElement>(null);

  // No need for loading or error states here, as data is pre-fetched by the server component
  // The error handling is now in BlogPostsServerWrapper.tsx
              ...
              <div className="relative w-full h-full">
                {node.featuredImage?.node?.sourceUrl && (
                  <Image
                    ...
                    width={600}
                    height={400}
                    style={{ objectFit: 'cover', width: 'auto', height: 'auto', maxWidth: '100%', maxHeight: '100%' }}
                    className="transition-transform duration-500 group-hover:scale-110"
                  />
                )}
                <div className="absolute inset-0 bg-gradient-to-t from-black/70 via-black/40 to-transparent" />
                <div className="absolute bottom-0 left-0 p-4 md:p-6 text-white">
                  <h3 className="text-lg md:text-xl font-bold">{node.title}</h3>
                  <div 
                    className="text-md md:text-lg font-semibold mt-1 line-clamp-2" 
